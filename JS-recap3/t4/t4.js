...
// TODO1. Etsi oma lokaatio
// MIETI: Onko listassa ja sen objekteissa aina koordinaatit, varsinkin jos tulee rajapinnan kautta
// TODO2. lasketaan ja lisätään etäisyys
// TODO3. sortataan lista jotta voidaan tulostaa se html sivulle

const yourLoc = document.getElementById('yourLoc');
...
const proceedWithLocation = (userLat, userLon) => {
  ...
  // MAP Lisää etäisyys jokaiselle (map tekee uuden taulukon)
  const restaurantwithDistance = clean.map((r) => {
    // hae jokaisen ravintolan lat ja lon
...
const onSuccess = (pos) => {
  ...
  yourLoc.textContent = `Sijaintisi: ${lat.toFixed(5)}, ${lon.toFixed(5)}`;
...
const onError = () => {
  ...
  let lat = 60.1699;
  let lon = 24.9384;
  yourLoc.textContent = `Sijainnin saaminen ei onnistunut, käytetään oletuksena Helsinki`;
  proceedWithLocation(lat, lon);
};
...
const init = () => {
  if (navigator.geolocation) {
    ...
    yourLoc.innerHTML = 'Geolocation ei ole tuettu tässä selaimessa';
    onError(); // käytetään fallbackia silti
  }
};

init();

// TÄSSÄ ALLA MUUTAMA NOPEA ESIMERKKI
...
// Filter lyhyesti
console.log(rest);
const restWithLocation = rest.filter((rivi) => rivi.location);
console.log(restWithLocation);
...
const restwithDistance = rest.map((rivi) => {
  return {...rivi, distance: 12.5};
});

console.log('Rest with distance', restwithDistance);

// Spread operaattori
const a = [1, 2, 3];
const b = [4, 5, 6];
const yhdistetty = [...a, 4];
console.log(yhdistetty); // [1, 2, 3, 4, 5, 6]
